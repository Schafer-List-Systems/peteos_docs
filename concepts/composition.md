...
The simplest agentic class derives directly from `AgenticObject`:
...
To enable multi-inheritance, each parent class in the hierarchy must also derive from `AgenticObject`. The system determines which classes in the inheritance chain are agentic by checking whether they directly inherit from `AgenticObject`.

## System Prompt Construction

When composing multiple agentic classes, the agent's system prompt is built by concatenating the docstrings of each agentic parent class in the Method Resolution Order (MRO). The most basic class's docstring (typically `AgenticObject`) comes first, and classes appearing later in the inheritance line are appended later in the combined prompt.
...
The most basic class (`AgenticObject`) contributes no prompt text (it has no docstring). `Navigator` comes first in the MRO, `InventoryManager` second, then the `Adventurer` prompt itself comes last. Invoking this agent:
